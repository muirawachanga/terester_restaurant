...
frappe.ui.form.on('Restaurant Order Entry', {
	setup: function(frm) {
		let get_item_query = () => {
			return {
				query: 'terester_restaurant.terester_restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				filters: {
...
//	    item is a field in table items
	...
	onload_post_render: function(frm) {
		if(!this.item_selector) {
			this.item_selector = new erpnext.ItemSelector({
				frm: frm,
				item_field: 'item',
				item_query: 'terester_restaurant.terester_restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				get_filters: () => {
	...
	restaurant_table: function(frm) {
		...
		return frappe.call({
			method: 'terester_restaurant.terester_restaurant.doctype.restaurant_order_entry.restaurant_order_entry.get_invoice',
			args: {
				table: frm.doc.restaurant_table
			},
			callback: (r) => {
	...
	sync: function(frm) {
		return frappe.call({
			method: 'terester_restaurant.terester_restaurant.doctype.restaurant_order_entry.restaurant_order_entry.sync',
			args: {
				table: frm.doc.restaurant_table,
				items: frm.doc.items
			},
	...
	make_invoice: function(frm) {
		frm.trigger('sync').then(() => {
			frappe.prompt([
				...
				return frappe.call({
					method: 'terester_restaurant.terester_restaurant.doctype.restaurant_order_entry.restaurant_order_entry.make_invoice',
					args: {
						table: frm.doc.restaurant_table,
						customer: data.customer,
						mode_of_payment: data.mode_of_payment,
						money: data.enter_amount
					},
					callback: (r) => {
